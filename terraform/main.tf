...
# Query all available AZs in the region
...
resource "aws_security_group" "test_instance_sg" {
  name_prefix = "${var.project_name}-test-instance-sg"
  vpc_id      = aws_vpc.main.id
  ...
  tags = { Name = "${var.project_name}-test-instance-sg" }
}
...
resource "aws_eip" "test_instance_eip" {
  ...
  tags = { Name = "${var.project_name}-test-instance-eip" }
}